...
export class AuthController {
  ...
  async login(@Request() req: any) {
    ...
    return { data: { ...instanceToPlain(req.user), token } };
  }
  ...
  async register(@Body() registerDto: RegisterDto) {
    ...
    return {
      data: { ...instanceToPlain(data.user), token: data.token },
    };
  }
  ...
  async socialLogin(@Body() socialLoginDto: SocialAuthDto) {
    const data = await this.authService.socialAuth(socialLoginDto);
    return {
      data: { ...instanceToPlain(data.user), token: data.token },
    };
  }
  ...
  async activate(
    ...
    const data = await this.authService.activateUser(userId, activateDto);
    return {
      data: { ...instanceToPlain(data.user), token: data.token },
    };
  }
